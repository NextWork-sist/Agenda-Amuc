...
async function actualizarReservaSeleccionada(){
  ...
  const senaObjetivo=Number(r.valor_total||0)*0.20;
  ...
  resumen.innerHTML=`
    ...
    <div><span>Seña requerida (20%)</span><strong>${money(senaObjetivo)}</strong></div>
...
function actualizarImporte(){
  ...
  if(concepto==='SENA'){
    ...
    help.textContent=pendienteSena>0?'Importe calculado automáticamente: 20% del alquiler.':'La seña requerida ya se encuentra abonada.';
